...
def _get_invite_user_blocks(user_id, channel_id, oauth_URI, message_ts=""):
    return [
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": _get_invite_string_for_user(user_id)},
        },
        {
            "type": "actions",
            "elements": [
...
def request_to_join_group(client, user_id, channel_name, oauth_URI):
    ...
    response = client.chat_postMessage(channel=group_to_join, blocks=invite_user_button)
    if not response["ok"]:
    ...
    response = client.chat_update(
        ...
        ts=message_id,
        blocks=_get_invite_user_blocks(
            user_requesting_to_join, group_to_join, oauth_URI, message_id
        ),
    )
    if not response["ok"]:
        raise AssertionError
...
def _replace_confirm_invite_button_with_success_message(client, channel, timestamp):
    user_invited_block = [
        {"type": "section", "text": {"type": "mrkdwn", "text": USER_INVITED_STRING}}
    ]
...
def invite_user_to_group(user_client, bot_client, oauth_state):
    invite_user_id, invite_channel_id, invite_message_ts = oauth_state.split(
        STATE_DIVIDER
    )
    ...
    _replace_confirm_invite_button_with_success_message(
        bot_client, invite_channel_id, invite_message_ts
    )
